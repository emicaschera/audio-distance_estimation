...
if __name__ == "__main__":
    ...
    config = {
        "max_epochs": 50,
        "batch_size": 16,
        "lr": 0.0001, #prova a ridurre per divergenza encoder 0.001 default
        "sampling_frequency": 16000,
        "dBNoise" : None, # None for noiseless, [50, 40, 30, 20, 10, 5, 0] for selecting the dB
        "kernels": "freq",
        "n_grus": 2,
        "features_set": "all",
        "att_conf": ["onAll"],
        "encoder_type": ["custom_free"], # ["param_sinc", "stft", "analytic_free", "free"]
        "n_filters": 512,
        "kernel_size": 512
    }
    ...
    if config["dBNoise"] == None:
        for conf in config["encoder_type"]:
                all_results = pd.DataFrame([], columns = ["GT", "Pred", "L1", "rL1", "ID"])
                run_name = "Kernels{}_Gru{}_Features_{}_Encoder_type_{}".format(config['kernels'], config['n_grus'], config['features_set'], conf )
                ...
                for i in range(0, 5):
                    ...
                    model = SeldTrainer(lr=config["lr"], kernels = config['kernels'], n_grus = config['n_grus'], features_set = config['features_set'], att_conf = config["att_conf"], n_filters = config['n_filters'], kernel_size = config['kernel_size'], encoder_type = conf, sampling_rate = config["sampling_frequency"])
